...
from marnies_maintenance_manager.jobs.views.agent_export_jobs_to_spreadsheet_view import (
...
def test_get_get_download_filename() -> None:
    ...
    assert (
        ...
        == "marnie_maintenance_jobs_for_bob_as_of_20220102_030405.csv"
    )
...
def test_get_download_content_disposition_header() -> None:
    ...
    assert get_download_content_disposition(agent_username, timestamp) == (
        'inline; filename="marnie_maintenance_jobs_for_bob_as_of_'
        '20220102_030405.csv"'
    )
...
def test_get_initial_http_response_inline_display() -> None:
    ...
    assert response["Content-Disposition"] == (
        'inline; filename="marnie_maintenance_jobs_for_alice_as_of_'
        "20230405_060708.csv\"; filename*=UTF-8''marnie_maintenance_"
...
def test_get_initial_http_response_non_inline_display() -> None:
    ...
    assert response["Content-Disposition"] == (
        'attachment; filename="marnie_maintenance_jobs_for_bob_as_of_20230405_'
        '060708.csv"'
    )
    assert response["X-Content-Type-Options"] == "nosniff"
    assert response["Cache-Control"] == "no-cache"
...
def test_get_initial_http_response_with_different_display_param() -> None:
    ...
    assert response["Content-Type"] == "text/csv"
    assert response["Content-Disposition"] == (
        'attachment; filename="marnie_maintenance_jobs_for_charlie_as_of_'
        '20230405_060708.csv"'
    )
    assert response["X-Content-Type-Options"] == "nosniff"
    assert response["Cache-Control"] == "no-cache"
...
class TestAgentExportJobsToSpreadsheetView:
    ...
    def test_none_agent_is_denied(
        ...
        Args:
            bob_agent_user (User): The agent user.
            marnie_user_client (Client): The non-agent user's client.
        """
        url = reverse(
            "jobs:agent_export_jobs_to_spreadsheet_view",
            kwargs={"pk": bob_agent_user.pk},
    ...
    def test_succeeds_when_jobs_for_agent(
        ...
        assert response["Content-Type"] == "text/csv"
        assert (
            'attachment; filename="marnie_maintenance_jobs_for_bob_as_of_'
