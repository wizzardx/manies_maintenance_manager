"""Tests for the refuse quote view."""
...
def test_gets_redirected_to_login_for_anonymous_user(client: Client) -> None:
    ...
    with pytest.warns(
        ...
        response = client.get(
            "/jobs/0dd3c34c-2003-11ef-8db1-5baa8ebc9c58/refuse-quote/",
            follow=True,
        )
    ...
    assert response.redirect_chain == [
        (
            "/accounts/login/?next=/jobs/0dd3c34c-2003-11ef-8db1-5baa8ebc9c58/refuse-quote/",
            302,
        ),
    ]
...
def test_gets_permission_error_for_marnie_user(
    ...
    """Ensure that Marnie cannot refuse the quote for Bob's job.
...
def test_gets_permission_error_for_different_agent_user(
    ...
    """Ensure that Peter cannot refuse the quote for Bob's job.
...
def test_fails_if_job_not_in_correct_state(
    ...
    """Ensure that Bob cannot refuse the quote if the job is not in the correct state.
    ...
    response = bob_agent_user_client.post(
        f"/jobs/{job_created_by_bob.id}/refuse-quote/",
    )
    assert response.status_code == status.HTTP_412_PRECONDITION_FAILED
    assert response.json() == {
        "error": "Job is not in the correct state for refusing a quote.",
    }
...
def test_redirects_to_job_detail_view(
    ...
    """Ensure user is redirected to the job detail view after refusing the quote.
...
def test_sets_job_to_refused_by_agent(
    bob_agent_user_client: Client,
    bob_job_with_initial_marnie_inspection: Job,
) -> None:
    """Ensure that the job is set to REFUSED_BY_AGENT after refusing the quote.
...
def test_email_sent_to_marnie_user(
    ...
    """Ensure that an email is sent to Marnie after Bob refuses the quote.
    ...
    # Check mail metadata:
    assert email.subject == "Quote refused by bob"
    ...
    # Check mail contents:
    assert "Agent bob has refused the quote for your maintenance request." in email.body
...
def test_flash_message_displayed(
    ...
    """Ensure that a flash message is displayed after refusing a quote.
    ...
    assert str(messages[0]) == "Quote refused. An email has been sent to Marnie."


def test_admin_user_can_refuse_quote(
    admin_client: Client,
    bob_job_with_initial_marnie_inspection: Job,
) -> None:
    """Ensure that the admin user can refuse the quote.
