...
in the Manie's Maintenance Manager application.
"""
...
from manies_maintenance_manager.functional_tests.utils.login import _sign_into_website
...
def test_manie_can_view_agents_job(
    browser: WebDriver,
    live_server_url: str,
    manie_user: User,
    bob_agent_user: User,
) -> None:
    """Ensure Manie can view the jobs submitted by Bob the Agent.
    ...
    # Manie received the notification email (in an earlier step), and so now he wants
    # to take a look at the Maintenance Job details
    # that were submitted by the Agent.

    # Manie logs into the system.
