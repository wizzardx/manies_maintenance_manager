...
from manies_maintenance_manager.jobs.constants import POST_METHOD_NAME
...
from .exceptions import ManieUserNotFoundError
from .exceptions import MultipleManieUsersError
...
def get_manie_email() -> str:
    """Return the email address for Manie.

    Returns:
        str: The email address for Manie.

    Raises:
        ManieUserNotFoundError: If no Manie user is found.
        MultipleManieUsersError: If multiple Manie users are found.
    """
    try:
        manie = User.objects.get(is_manie=True)
    except User.DoesNotExist as err:
        raise ManieUserNotFoundError from err
    except User.MultipleObjectsReturned as err:
        raise MultipleManieUsersError from err
    return manie.email
...
def count_manie_users() -> int:
    """Return the number of Manie users.

    Returns:
        int: The number of Manie users.
    """
    return User.objects.filter(is_manie=True).count()
...
def quote_accept_or_reject(
    ...
    *,
    ...
    # Send email to Manie telling him that his quote was accepted by the agent
    ...
    email_body = (
        ...
        f"Manie performed the inspection on {job.date_of_inspection} and has "
        "quoted you.\n\n"
    )
    ...
    messages.success(request, f"Quote {verb}ed. An email has been sent to Manie.")
