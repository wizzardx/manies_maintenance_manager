...
def agent_list(request: HttpRequest) -> HttpResponse:
    ...
    Or more precisely, we want to - for the benefit of Manie - map between each
    ...
    """
    # Only Manie user and the Admin may access this view. Return a 403 Forbidden
    # response if some other user is trying to access this view.
    user = check_type(request.user, User)
    if not (user.is_superuser or user.is_manie):
