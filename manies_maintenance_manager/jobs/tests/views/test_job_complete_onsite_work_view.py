...
from manies_maintenance_manager.jobs.views.job_complete_onsite_work_view import (
...
def test_view_is_accessible_by_manie_when_pop_has_been_uploaded(
    bob_job_with_deposit_pop: Job,
...
def test_updating_job_changes_status_to_completed(
    ...
    assert (
        bob_job_with_deposit_pop.status == Job.Status.MANIE_COMPLETED_ONSITE_WORK.value
    )
...
def test_manie_cannot_access_view_after_completing_the_job(
    ...
    Args:
        bob_job_with_onsite_work_completed_by_manie (Job): Job that's had the onsite
            work completed by Manie.
        manie_user_client (Client): The Django test client for Manie.
    """
    url = reverse(
        "jobs:job_complete_onsite_work",
...
def test_manie_clicking_save_sends_an_email_to_agent(
    ...
    # Check mail metadata:
    assert email.subject == "Manie completed onsite work on a maintenance job"
    ...
    # Check mail contents:
    assert (
        "Manie completed the onsite maintenance work on 2022-03-04. "
