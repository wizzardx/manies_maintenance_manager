...
class TestManieAccessingJobListView:
    """Test Manie's ability to access and filter job listings."""
    ...
    def test_with_agent_username_url_param_filters_by_agent(
        ...
        """Test filtering job list by agent for Manie with agent username parameter.

        Args:
            bob_agent_user (User): The agent user Bob whose jobs are to be filtered.
            job_created_by_bob (Job): A job instance created by Bob.
            manie_user_client (Client): A test client used by Manie to view the job
                                         list.
        """
    ...
    def test_without_agent_username_url_param_returns_bad_request_response(
        ...
        Args:
            job_created_by_bob (Job): A job instance created for Bob.
            manie_user_client (Client): A test client used by Manie.
        """
        response = manie_user_client.get(reverse("jobs:job_list"))
    ...
    def test_create_maintenance_job_button_present_for_agent_but_not_for_manie(
        self,
        bob_agent_user_client: Client,
        manie_user_client: Client,
        bob_agent_user: User,
        ...
        # But that it isn't present for Manie:
        response = manie_user_client.get(
            reverse("jobs:job_list") + f"?agent={bob_agent_user.username}",
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_number_cell_is_a_link_to_the_job_details(  # pylint: disable=too-many-locals
        ...
        # Manie can see a link in the "Number" cell of the table.
...
def test_job_list_view_contains_basic_usage_advice(
    ...
    Args:
        superuser_client (Client): A test client with superuser permissions.
        bob_job_with_manie_final_documentation (Job): A job created by Bob with the
            final documentation uploaded by Manie.
    """
    response = superuser_client.get(reverse("jobs:job_list"))
    assert response.status_code == status.HTTP_200_OK
    assert (
...
class TestTipShownForManieIfThereAreNoJobsListed:
    """Test the message shown to Manie when no jobs are listed."""
    ...
    def test_msg_not_shown_if_not_manie(self, bob_agent_user_client: Client) -> None:
    ...
    def test_msg_not_shown_if_manie_and_jobs_exist(
        self,
        manie_user_client: Client,
        job_created_by_bob: Job,
    ) -> None:
        """Ensure the correct message is not shown for Manie when jobs exist.
    ...
    def test_msg_shown_if_manie_and_no_jobs_exist(
        self,
        manie_user_client: Client,
        bob_agent_user: User,
    ) -> None:
...
class TestTipShownForAllUsersIfThereAreAnyJobsListed:
    ...
    def test_msg_shown_if_there_is_at_least_one_job(
        ...
        Args:
            manie_user_client (Client): A test client used by Manie.
            bob_job_with_manie_final_documentation (Job): A job where Manie has
...
class TestExportToSpreadsheetLink:
    ...
    def test_visible_for_agent(
        ...
        Args:
            bob_agent_user_client (Client): A test client for agent user Bob.
            bob_job_with_manie_final_documentation (Job): A job created by Bob with
                final documentation uploaded by Manie.
        """
        response = bob_agent_user_client.get(reverse("jobs:job_list"))
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_not_visible_for_manie(
        ...
        """Test that the "Export to Spreadsheet" link is not visible for Manie.
        ...
        response = manie_user_client.get(
            ...
            + f"?agent={bob_job_with_manie_final_documentation.agent.username}",
        )
        assert response.status_code == status.HTTP_200_OK
