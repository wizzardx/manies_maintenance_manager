...
from manies_maintenance_manager.jobs.views.quote_upload_view import QuoteUploadView
...
def test_agent_user_cannot_access_the_view(
    ...
    Args:
        ...
        bob_job_upload_quote_url (str): The URL for the view where Manie can
...
def test_manie_user_can_access_the_view(
    ...
    """Test that the Manie user can access the "upload quote" view.

    Args:
        manie_user_client (Client): The Django test client for Manie.
        bob_job_upload_quote_url (str): The URL for the view where Manie can upload a
            quote for the job created by Bob.
    """
    response = manie_user_client.get(bob_job_upload_quote_url)
    assert response.status_code == status.HTTP_200_OK
...
def test_view_has_quote_field(
    ...
    quote_name = bob_job_with_initial_manie_inspection.quote.name
...
def test_updating_job_changes_status_to_quote_uploaded(
    ...
    bob_job_with_initial_manie_inspection.refresh_from_db()
    assert (
        bob_job_with_initial_manie_inspection.status == Job.Status.QUOTE_UPLOADED.value
    )
...
def test_manie_cannot_access_view_after_quote_already_uploaded(
...
def test_superuser_cannot_access_view_after_quote_already_uploaded(
    ...
    """Ensure the superuser can't access the view after Manie already uploaded a quote.
...
def http_response_to_manie_uploading_a_quote(
    ...
    """Get the HTTP response after Manie uploads an Invoice to Bob's job.
    ...
    Returns:
        TemplateResponse: The HTTP response after Manie uploads an Invoice to Bob's
            job.
    """
    with safe_read(test_pdf):
        response = check_type(
...
def flashed_message_after_uploading_a_quote(
    ...
    """Retrieve the flashed message after Manie uploads a quote.

    Args:
        http_response_to_manie_uploading_a_quote (TemplateResponse): The
            HTTP response after Manie uploads a quote.

    Returns:
        Message: The flashed message after Manie uploads a quote.
    """
    # Check the messages:
    response = http_response_to_manie_uploading_a_quote
...
def test_manie_clicking_save_sends_an_email_to_agent(
    ...
    Args:
        http_response_to_manie_uploading_a_quote (TemplateResponse): The HTTP response
            after Manie uploads a quote.
    ...
    # Check mail metadata:
    assert email.subject == "Manie uploaded a quote for your maintenance request"
    ...
    # Check mail contents:
    assert (
        "Manie uploaded a quote for a maintenance job. "
...
def test_manie_clicking_save_sets_state_to_quote_uploaded(
    bob_job_with_initial_manie_inspection: Job,
    manie_user_client: Client,
    bob_job_upload_quote_url: str,
    test_pdf: SimpleUploadedFile,
) -> None:
    """Test that Manie clicking 'Save' sets the state to 'Quote Uploaded'.
