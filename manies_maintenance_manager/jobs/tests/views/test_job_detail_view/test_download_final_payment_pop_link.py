...
Manie's Maintenance Manager application. It ensures various user roles and scenarios
...
- Specific business roles like Manie or an admin user have the appropriate visibility.
...
Functions:
    ...
    test_agent_who_did_not_create_job_cannot_reach_page_to_see_link: Restrict
        unauthorized agents.
    test_manie_can_see_link: Confirm that Manie can see the POP link.
...
def test_manie_can_see_link(
    ...
    """
    job = bob_job_with_final_payment_pop
    link = get_final_payment_pop_link(manie_user, job)
    assert link is not None
    assert link["href"] == job.final_payment_pop.url
