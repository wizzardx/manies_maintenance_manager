...
from manies_maintenance_manager.jobs.tests.conftest import BASIC_TEST_PDF_FILE_2
from manies_maintenance_manager.jobs.utils import safe_read
from manies_maintenance_manager.jobs.views.quote_update_view import QuoteUpdateView
...
def test_on_success_sends_an_email_to_the_agent(
    ...
    # Check mail metadata:
    assert email.subject == "Manie uploaded an updated quote for your job"
    ...
    # Check mail contents:
    assert (
        "Manie uploaded a new quote for a maintenance job. The quote "
...
def test_does_not_work_if_not_in_quote_rejected_state(
    ...
    Args:
        bob_job_with_initial_manie_inspection (Job): A Job instance created by Bob,
            with an initial Manie inspection.
        manie_user_client (Client): The Django test client for Manie.
        test_pdf_2 (SimpleUploadedFile): A test PDF file.
    """
    submit_quote_update(
