"""Tests for HTML content validation in Manie's Maintenance Manager application views.

This module checks the HTML content returned by various views in the Manie's
...
def assert_standard_email_content(email: EmailMessage, job: Job) -> None:
    ...
    assert (
        "Manie performed the inspection on 2001-02-05 and has quoted you." in email.body
    )
...
def assert_standard_quote_post_response(
    ...
    Args:
        ...
        bob_job_with_initial_manie_inspection (Job): The job with an initial Manie
            inspection.
    ...
    response = bob_agent_user_client.post(
        f"/jobs/{bob_job_with_initial_manie_inspection.id}/quote/{verb}/",
        follow=True,
    )
    ...
    assert response.redirect_chain == [
        (f"/jobs/{bob_job_with_initial_manie_inspection.id}/", 302),
    ]
    # Check the message:
