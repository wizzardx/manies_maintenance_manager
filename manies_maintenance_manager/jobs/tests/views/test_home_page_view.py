...
from manies_maintenance_manager.jobs.tests.views.utils import HTTP_SUCCESS_STATUS_CODE
from manies_maintenance_manager.jobs.tests.views.utils import (
    check_basic_page_html_structure,
)
from manies_maintenance_manager.jobs.utils import count_admin_users
from manies_maintenance_manager.jobs.utils import count_agent_users
from manies_maintenance_manager.jobs.utils import count_manie_users
from manies_maintenance_manager.jobs.utils import get_test_user_password
from manies_maintenance_manager.jobs.views.home_page_view import (
...
class TestBasicHomePageText:
    ...
    def test_basic_welcome_text(self, client: Client) -> None:
        ...
        assert "Welcome to Manie's Maintenance Manager!" in str(
    ...
    def test_not_signed_in(self, client: Client) -> None:
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            "Please Sign In to the system to book a home visit by Manie."
    ...
    def test_manie_signed_in(self, manie_user_client: Client) -> None:
        ...
        Args:
            manie_user_client (Client): A test client for user Manie.
        """
        response = manie_user_client.get(reverse("home"))
        assert response.status_code == status.HTTP_200_OK
        assert (
            'Welcome back, Manie. You can click on the "Agents" link above, to see '
    ...
    def test_agent_signed_in(self, bob_agent_user_client: Client) -> None:
        ...
        assert (
            ...
            "the list of Maintenance Visits scheduled for Manie."
            in response.content.decode()
        )
    ...
    def test_unknown_user_signed_in(self, unknown_user_client: Client) -> None:
        ...
        assert (
            "If you're a property agent, then please contact Manie so that this "
...
def test_home_page_returns_correct_html(client: Client) -> None:
    ...
    check_basic_page_html_structure(
        client=client,
        url="/",
        expected_title="Manie's Maintenance Manager",
...
class TestAdminSpecificHomePageWarnings:
    ...
    # Tests for when there is no Manie user in the system

    def test_warning_for_no_manie_user(self, admin_client: Client) -> None:
        """Test that a warning is shown when there are no Manie users.

        Args:
            admin_client (Client): A test client with admin privileges.
        """
        # Make sure there are no Manie users here
        assert count_manie_users() == 0

        response = admin_client.get("/")
        assert response.status_code == HTTP_SUCCESS_STATUS_CODE
        assert (
            USER_COUNT_PROBLEM_MESSAGES["NO_MANIE_USERS"] in response.content.decode()
        )

    def test_no_warning_for_no_manie_user_when_there_is_a_manie_user(
        self,
        manie_user: User,
        admin_client: Client,
    ) -> None:
        """Ensure no warning for no Manie users when there is a Manie user.

        Args:
            manie_user (User): User instance representing Manie.
            admin_client (Client): A test client with admin privileges.
        """
        # Make sure there is at least one Manie user
        assert count_manie_users() >= 1

        response = admin_client.get("/")
        assert response.status_code == HTTP_SUCCESS_STATUS_CODE
        assert (
            USER_COUNT_PROBLEM_MESSAGES["NO_MANIE_USERS"]
            not in response.content.decode()
        )

    def test_no_warning_for_no_manie_user_when_i_am_not_admin(
        self,
        bob_agent_user_client: Client,
    ) -> None:
        """Test there is no warning for no Manie users when the user is not an admin.
    ...
    # Tests for when there are multiple Manie users.

    def test_warning_for_multiple_manie_users(
        self,
        manie_user: User,
        bob_agent_user: User,
        admin_client: Client,
    ) -> None:
        """Test that a warning is shown when there are multiple Manie users.

        Args:
            admin_client (Client): A test client with admin privileges.
            bob_agent_user (User): User instance for Bob, temporarily made a Manie
                                   user for this test.
            manie_user (User): User instance representing Manie.
        """
        # Make sure there are at least 2 Manie users here
        bob_agent_user.is_manie = True
        bob_agent_user.save()
        assert count_manie_users() >= 2  # noqa: PLR2004

        response = admin_client.get("/")
        assert response.status_code == HTTP_SUCCESS_STATUS_CODE
        assert (
            USER_COUNT_PROBLEM_MESSAGES["MANY_MANIE_USERS"] in response.content.decode()
        )

    def test_no_warning_for_multiple_manie_users_when_there_is_one_manie_user(
        self,
        manie_user: User,
        admin_client: Client,
    ) -> None:
        """Ensure no warning for multiple Manie users when there is one Manie user.

        Args:
            manie_user (User): User instance representing Manie.
            admin_client (Client): A test client with admin privileges.
        """
        # Make sure there is only one Manie user
        assert count_manie_users() == 1

        response = admin_client.get("/")
        assert response.status_code == HTTP_SUCCESS_STATUS_CODE
        assert (
            USER_COUNT_PROBLEM_MESSAGES["MANY_MANIE_USERS"]
            not in response.content.decode()
        )

    def test_no_warning_for_multiple_manie_users_when_i_am_not_admin(
        self,
        manie_user: User,
        bob_agent_user: User,
        bob_agent_user_client: Client,
    ) -> None:
        """Ensure no Manie user multiple warning when not an admin.

        Args:
            bob_agent_user (User): User instance for Bob, marked as a Manie user.
            bob_agent_user_client (Client): A test client for Bob, who is an agent but
                                            not an admin.
            manie_user (User): Another user instance flagged as Manie to simulate the
                                test condition.
        """
        # Make sure there are at least 2 Manie users here
        # We have a single 'manie' user already, lets flag 'bob' as a 'manie' user,
        # too.
        bob_agent_user.is_manie = True
        bob_agent_user.save()
...
def test_maintenance_jobs_link_in_navbar_is_not_present_for_manie_user(
    client: Client,
    manie_user: User,
) -> None:
    """Check 'Maintenance Jobs' link is not visible for non-agent user Manie.
...
def test_agents_link_is_visible_for_manie_user(
    client: Client,
    manie_user: User,
) -> None:
    """Check that the 'Agents' link is visible for Manie.

    Args:
        client (Client): Django's test client instance used for making requests.
        manie_user (User): User instance representing Manie, who is not an agent.
    """
    assert _login_and_check_agents_link(client, "manie")


def test_agents_link_is_not_visible_for_none_manie_users(
    client: Client,
    bob_agent_user: User,
) -> None:
