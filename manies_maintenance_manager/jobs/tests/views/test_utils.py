...
from manies_maintenance_manager.jobs.tests.views import utils
from manies_maintenance_manager.jobs.views import utils as views_utils
...
class TestAssertNoFormErrors:
    ...
    def test_form_errors_cause_assertion_error(mocker: pytest_mock.MockFixture) -> None:
        ...
        mocker.patch("manies_maintenance_manager.jobs.tests.views.utils.isinstance")
...
def test_prepare_and_send_email_with_unknown_attachment_type(
    ...
    Args:
        mocker (pytest_mock.MockFixture): A pytest-mock fixture.
        manie_user (User): A User instance
    """
    with pytest.raises(
        ValueError,
        match=re.escape(
...
def test_prepare_and_send_email_quote(mocker: pytest_mock.MockFixture) -> None:
    ...
    mocker.patch.object(
        views_utils,
        "get_manie_email",
        return_value="manie@example.com",
    )
