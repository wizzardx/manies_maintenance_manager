...
from manies_maintenance_manager.jobs import exceptions
from manies_maintenance_manager.jobs import utils
from manies_maintenance_manager.jobs.models import Job
from manies_maintenance_manager.jobs.tests import utils as test_utils
...
class TestGetManieEmail:
    ...
    def test_gets_manie_user_email(self, manie_user: User) -> None:
        """Test that the email address for Manie is returned.

        Args:
            manie_user (User): A user instance representing Manie, expected to be
                                queried.
        """
        expected = manie_user.email
        assert utils.get_manie_email() == expected

    def test_fails_when_no_manie_user(self) -> None:
        """Test that an exception is raised when there is no Manie user."""
        with pytest.raises(
            exceptions.ManieUserNotFoundError,
            match="No Manie user found",
        ):
            utils.get_manie_email()

    def test_fails_when_multiple_manie_users(
        self,
        manie_user: User,
        bob_agent_user: User,
    ) -> None:
        """Test that an exception is raised when there are multiple Manie users.

        Args:
            manie_user (User): A user instance representing Manie.
            bob_agent_user (User): Another user instance incorrectly flagged as Manie.
        """
        # Grab the bob user, and set his is_manie to True, to help to trigger this
        # test case.
        bob_agent_user.is_manie = True
        bob_agent_user.save()

        with pytest.raises(
            exceptions.MultipleManieUsersError,
            match="Multiple Manie users found",
        ):
            utils.get_manie_email()
...
class TestGetSystemAdministratorEmail:
    ...
    def test_works_when_there_are_multiple_sysadmin_users(
        ...
        Args:
            ...
            manie_user (User): Another admin user to simulate multiple admins.
            caplog (pytest.LogCaptureFixture): Fixture to capture log outputs.
        """
        # Make Manie into a sysadmin, in addition to our testing "admin" account,
        ...
        # about this.
        manie_user.is_superuser = True
        manie_user.save()

        email = utils.get_sysadmin_email()
        assert email in [admin_user.email, manie_user.email]
...
class TestFirstOrError:
    ...
    # noinspection PyUnusedLocal
    def test_gets_first_object(self, admin_user: User, manie_user: User) -> None:
        """Test that the first object in a queryset is returned.

        Args:
            admin_user (User): An admin user included in the queryset.
            manie_user (User): Another user included in the queryset.
        """
...
@mock.patch("manies_maintenance_manager.jobs.views.utils.EmailMessage")
@mock.patch("manies_maintenance_manager.jobs.views.utils.get_manie_email")
@mock.patch("manies_maintenance_manager.jobs.views.utils.generate_email_body")
def test_send_quote_update_email(
    mock_generate_email_body: mock.Mock,
    mock_get_manie_email: mock.Mock,
    ...
    Args:
        ...
        mock_get_manie_email (mock.Mock): Mock of the get_manie_email function.
    ...
    mock_get_manie_email.return_value = "manie@mmm.ar-ciel.org"
    ...
    mock_get_manie_email.assert_called_once()
...
@mock.patch("manies_maintenance_manager.jobs.utils.messages.success")
def test_quote_accept_or_reject_skip_email_send(
    ...
    mock_get_manie_email.return_value = "manie@example.com"
    mock_generate_email_body.return_value = "Generated email body"
    ...
    mock_messages_success.assert_called_once_with(
        http_request,
        "Quote accepted. An email has been sent to Manie.",
    )
